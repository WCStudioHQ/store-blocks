...
import { TabPanel, Panel, PanelBody, RangeControl, SelectControl, ToggleControl, FontSizePicker, ColorPicker, TextControl,__experimentalUnitControl as UnitControl } from '@wordpress/components';
...
export default function Edit({ attributes, setAttributes }) {
	...
	return (
		<div>
			<InspectorControls>
				<TabPanel
					className="product-table-tabs"
					activeClass="is-active"
					tabs={[
						{
							name: 'settings',
							title: __('Settings', 'store-blocks'),
							className: 'tab-settings',
						},
						{
							name: 'style',
							title: __('Style', 'store-blocks'),
							className: 'tab-style',
						},
					]}
				>
									...
									{attributes.showTableCaption && (
										<div>
											...
											<UnitControl
												...
												units={['px', 'em', 'rem'].map((unit) => ({ value: unit, label: unit }))}
			...
			<div {...blockProps} >
				{products.length > 0 ? (
					<table className="store-blocks-product-table">
						{attributes.showTableCaption && (
							<caption style={{ fontSize: attributes.captionFontSize, color: attributes.captionColor, marginBottom: attributes.captionSpacing }}>
